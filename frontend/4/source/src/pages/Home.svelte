<script>
  ...
  function getProductList() {
    ...
    url += `&start=${start}&count=${PAGE_SIZE}`;
  ...
  <div class="row">
    ...
    <div class="col-9">
      ...
      <nav aria-label="Page navigation example">
        <ul class="pagination">
          <li class="page-item">
            <a on:click={() => setPage(1)} class="page-link" href="#/">&laquo;</a>
          </li>
          <li class="page-item">
            <a on:click={() =>{if(page>1)setPage(page-1)}} class="page-link" href="#/">&lsaquo;</a>
          </li>
          <li class="page-item">
            <a on:click={() =>{if(page<numPage)setPage(page+1)}} class="page-link" href="#/">&rsaquo;</a>
          </li>
          <li class="page-item">
            <a on:click={() => setPage(numPage)} class="page-link" href="#/">&raquo;</a>
          </li>
        </ul>
        <span>Hiển thị {start+1??''}-{end??''} trên tổng số {total??''} sản phẩm</span>
      </nav>
    </div>
  </div>
</div>
